...
func _init():
	display_name = "Dampen"
	description = "At the start of each turn, decreases ATK of adjacent enemy units permenantly by 2."
	value = 2
	range = 1
	trigger = "passive"
	print("✅ Dampen initialized.")
# Called once when the unit enters the field or at each turn tick

func execute(arena: Node, unit: UnitData) -> void:
	print("🧊 Dampen.execute() called for", unit.card.name)
	...
	for d in [Vector2i(1,0), Vector2i(-1,0), Vector2i(0,1), Vector2i(0,-1)]:
		...
		if target.owner != unit.owner:
			target.current_atk = max(0, target.current_atk - value)
			arena._log("❄️ %s's Dampen soaks %s (-%d ATK)" % [
