...
# NODES
# -----------------------------
@onready var board: Board3DTutorial = $Board3D

@onready var ui_root: Control = $UISystemTutorial
@onready var camera: Camera3D = $CameraSystem


# Subsystems:
@onready var camera_sys: Node = $CameraSystem
@onready var ui_sys: Node = $UISystemTutorial
@onready var battle_sys: Node = $BattleSystem
@onready var ai_sys: Node = $AISystemTutorial
@onready var cutscene_sys: Node = $CutsceneSystem

# UI children we still reference directly (for convenience)
@onready var card_draw: AudioStreamPlayer = $UISystemTutorial/SFX/CardDraw
@onready var card_details_ui: Control = $UISystemTutorial/ArenaCardDetails
...
var tutorial_waiting_for_attack_choice := false
...
func _try_place_regular_card(hover_tile: Node3D) -> void:
	...
	ui_sys.show_popup(selected_card, func(mode):
		...
		ui_sys._on_player_pressed_continue()
...
func on_leader_defeated(owner: int) -> void:
	if owner == PLAYER:
		_log("This is only practice! Try again.", Color(1,0.3,0.3))
		restart_level()
	else:
		_log("Excellent! You've mastered the basics.", Color(0.3,1,0.3))
		ui_sys.call("show_battle_message", "Tutorial Complete!", 3.0)
		emit_signal("battle_finished", "tutorial_won")
