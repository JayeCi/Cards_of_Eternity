...
func _move_or_battle(from: Vector2i, to: Vector2i, bypass_control_check := false) -> void:
	...
	if (dst.occupant and attacker.card.card_type == CardData.CardType.MONSTER and
		dst.occupant.card and dst.occupant.card.is_upgrade_spell):
		clear_highlights()
		...
		# STEP 5: ✅ Refresh monster tile visuals after upgrade
		# Reassign occupant to ensure proper tile state
		dst.set_occupant_no_summon(attacker)
		...
		# Update stat labels
		if dst.has_method("update_stat_labels"):
			dst.update_stat_labels(attacker.current_atk, attacker.current_def)

		# Ensure model is visible if face-up
		if dst.has_node("CardModel"):
			var models = dst.get_node("CardModel")
			if is_instance_valid(models):
				models.visible = not attacker.is_facedown

		# Force tile refresh for proper clickability
		await get_tree().process_frame
	...
	if (dst.occupant and attacker.card.is_upgrade_spell and
		dst.occupant.card.card_type == CardData.CardType.MONSTER):
		clear_highlights()
		var monster_unit = dst.occupant
		await _try_upgrade_monster(attacker, monster_unit, src, dst)

		# ✅ Refresh monster tile visuals after upgrade
		# Reassign occupant to ensure proper tile state
		dst.set_occupant_no_summon(monster_unit)

		dst.set_art(
			monster_unit.card.art if monster_unit.mode != UnitData.Mode.FACEDOWN else core.CARD_BACK,
			monster_unit.owner == core.ENEMY
		)
		dst.refresh_card_art()

		# Update stat labels
		if dst.has_method("update_stat_labels"):
			dst.update_stat_labels(monster_unit.current_atk, monster_unit.current_def)

		# Ensure model is visible if face-up
		if dst.has_node("CardModel"):
			var model = dst.get_node("CardModel")
			if is_instance_valid(model):
				model.visible = not monster_unit.is_facedown
		...
		src.clear()
		core.units.erase(from)
		core.mark_unit_acted(monster_unit)  # Mark monster as acted, not the spell
		battle._is_battle_in_progress = false
		return

	# Case 3: Upgrade spell moves onto another spell or event - destroy the upgrade spell
	if (dst.occupant and attacker.card.is_upgrade_spell and dst.occupant.card and
		(dst.occupant.card.card_type == CardData.CardType.SPELL or
		dst.occupant.card.card_type == CardData.CardType.EVENT)):
	...
	# 🧬 BOARD-TO-BOARD FUSION: Check if moving friendly monster onto another friendly monster
	if (dst.occupant and attacker.owner == dst.occupant.owner and
		attacker.card.card_type == CardData.CardType.MONSTER and
		dst.occupant.card.card_type == CardData.CardType.MONSTER):
		# Check if this is a valid fusion
		...
		if fusion_result:
			...
			var fused_unit := UnitData.new().init_from_card(fusion_result, attacker.owner)
			fused_unit.mode = UnitData.Mode.ATTACK
			fused_unit.is_facedown = false
			...
			# Update stat labels
			if dst.has_method("update_stat_labels"):
				dst.update_stat_labels(fused_unit.current_atk, fused_unit.current_def)

			# Load 3D model if available
			if fusion_result.model_path != "":
				var model_scene: PackedScene = load(fusion_result.model_path)
				if model_scene:
					var model_instance: Node3D = model_scene.instantiate()
					model_instance.name = "CardModel"
					...
					dst.add_child(model_instance)
					fused_unit.set_meta("model_instance", model_instance)
					if attacker.owner == core.ENEMY:
						model_instance.rotate_y(deg_to_rad(180))
					model_instance.visible = true
	...
	for dir in dirs:
		for step in range(1, range + 1):
			...
			if t.occupant:
				...
				if (moving_is_monster and target_is_upgrade_spell and
					t.occupant.owner == core.PLAYER and not unit.is_leader):
				...
				elif (is_upgrade_spell and target_is_monster and
					t.occupant.owner == core.PLAYER and not t.occupant.is_leader):
					move_targets[p] = {"kind": "upgrade"}
					_highlight_move_tile(t, false, true)  # Yellow
					break

				# Case 3: Upgrade spell moving onto spell/event (YELLOW highlight - will be destroyed)
				elif (is_upgrade_spell and (target_is_spell or target_is_event) and
					t.occupant.owner == core.PLAYER):
					move_targets[p] = {"kind": "upgrade"}
					_highlight_move_tile(t, false, true)  # Yellow
					break

				# 🧬 Case 4: Monster moving onto friendly monster (check for fusion) - GREEN highlight
				elif (moving_is_monster and target_is_monster and
					t.occupant.owner == core.PLAYER and
					not unit.is_leader and not t.occupant.is_leader):
					# Check if this is a valid fusion
					var fusion_result = core._check_fusion_pair(unit.card, t.occupant.card)
					if fusion_result:
						move_targets[p] = {"kind": "fusion"}
						_highlight_move_tile(t, false, false, true)  # Green
						break
...
func _highlight_move_tile(tile: Node3D, is_attack: bool, is_upgrade: bool = false, is_fusion: bool = false) -> void:
	...
	if tile.has_method("set_highlight"):
		var symbol := "⚔" if is_attack else ("⚡" if is_upgrade else ("🧬" if is_fusion else "•"))
		tile.set_highlight(true, symbol)

	# Tint color
	var tint := Color(1.0, 1.0, 0.3) if is_upgrade else (Color(0.3, 1.0, 0.5) if is_fusion else (Color(1.0, 0.3, 0.3) if is_attack else Color(0.3, 0.7, 1.0)))
