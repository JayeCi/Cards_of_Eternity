...
func set_current_node(node: MapNode3D):
	...
	if current_node:
		...
		# DON'T auto-complete nodes! Only battles/encounters should mark completion
		# current_node.is_completed = true  # REMOVED - this was breaking everything
		...
		# Only reveal connected nodes if this node is already completed OR is a hub
		if current_node.is_completed or current_node.encounter_type == "hub":
			reveal_connected_nodes()
		else:
			print("  🔒 Node not completed yet, connected nodes remain locked")
...
func reveal_connected_nodes():
	...
	print("  📋 Current node is_completed: ", current_node.is_completed)
